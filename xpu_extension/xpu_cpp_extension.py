...
class DpcppBuildExtension(build_ext, object):
    ...
    def build_extensions(self) -> None:
        ...
        def append_std17_if_no_std_present(cflags) -> None:
            cpp_format_prefix = ("/{}:" if self.compiler.compiler_type
                                 == "msvc" else "-{}=")
        ...
        def _gen_link_lib_cmd_line(
            ...
            runtime_library_dirs_args += [
                f"-L{x}" for x in runtime_library_dirs
            ]
        ...
        def unix_wrap_ninja_compile(
            ...
            _write_ninja_file_and_compile_objects(
                ...
                objects=objects,
                cflags=[
                    shlex.quote(f) for f in extra_cc_cflags + common_cflags
                ],
        ...
        def unix_wrap_ninja_link_shared_object(
            ...
            # print('@@@@@@@@@@@@@@' + ldflags)
            _write_ninja_file_and_link_library(objects,
                                               output_libname,
                                               build_directory,
                                               ldflags,
                                               verbose=True)
...
def check_compiler_ok_for_platform(compiler: str) -> bool:
    ...
    compiler_path = os.path.realpath(
        which.decode(*SUBPROCESS_DECODE_ARGS).strip())

    # Check the compiler name
    if any(name in compiler_path
    ...
    version_string = subprocess.check_output(
        [compiler, "-v"],
        stderr=subprocess.STDOUT).decode(*SUBPROCESS_DECODE_ARGS)
    if IS_LINUX:
...
def check_compiler_abi_compatibility(compiler) -> bool:
    ...
    try:
        if IS_LINUX:
            ...
            version = ["0", "0", "0"] if match is None else list(
                match.groups())
    except Exception:
...
def _write_ninja_file_and_link_library(objects, output_libname,
                                       build_directory, ldflags,
                                       verbose: bool) -> None:
    build_file_path = os.path.join(build_directory, "build.ninja")
    if verbose:
        print(f"Emitting ninja build file {build_file_path}...")
...
def _write_ninja_file_and_build_library(
    ...
    extra_ldflags = _prepare_ldflags(extra_ldflags or [], verbose,
                                     is_standalone)
    ...
    _run_ninja_build(build_directory,
                     verbose,
...
def _prepare_ldflags(extra_ldflags, verbose, is_standalone):
    ...
    oneapi_link_args += [
        f"{x}" for x in get_one_api_help().get_onemkl_libraries()
    ]
...
def _run_ninja_build(build_directory: str, verbose: bool,
...
def _get_build_directory(name: str, verbose: bool) -> str:
    ...
    if root_extensions_directory is None:
        ...
        root_extensions_directory = os.path.join(root_extensions_directory,
                                                 build_folder)
...
def _write_ninja_file_to_build_library(path, name, sources, extra_cflags,
    ...
    # ]
    common_cflags += ["-D_GLIBCXX_USE_CXX11_ABI=" + str(1)]
    if IS_WINDOWS:
    ...
    ldflags = ([] if is_standalone else [SHARED_FLAG]) + [SYCL_FLAG
                                                          ] + extra_ldflags
    print('#############' + ldflags)
        ...
        if baton.try_acquire():
            try:
                with GeneratedFileCleaner(
                        keep_intermediates=keep_intermediates) as clean_ctx:
...
def _write_ninja_file(path, cflags, post_cflags, sources, objects, ldflags,
                      library_target) -> None:
