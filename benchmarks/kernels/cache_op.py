...
class CacheCopyBench(KernelBenchmark):
    ...
    def _run(self):
        for i in range(self.loop_time):
            cache_ops.copy_blocks(self.key_caches, self.value_caches, self.block_mapping)
...
class CacheReshapeBench(KernelBenchmark):
    ...
    def _run(self):
        cache_ops.reshape_and_cache(self.key, self.value, self.key_cache,
                          self.value_cache, self.slot_mapping)


# bench = CacheCopyBench(10, 256, 8, 16, 256, 16, 1024, torch.float16, torch.device("xpu"))
# bench.execute()
...
bench = CacheReshapeBench(10, 128, 64, 256, 16, 1024, torch.float16, torch.device("xpu"))
bench.execute()
