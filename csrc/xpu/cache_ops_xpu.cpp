...
void reshape_and_cache_xpu_impl(
    ...
    const int key_cache_stride, const int key_cache_num_tokens) {
  const int block_elem_num = num_heads * head_size * block_size;

  sycl::queue q(sycl::gpu_selector_v);
  sycl::buffer<scalar_t> key_buf(key, num_tokens * key_stride);
  sycl::buffer<scalar_t> value_buf(value, num_tokens * value_stride);
  sycl::buffer<int64_t> slot_mapping_buf(slot_mapping, num_tokens);

  sycl::buffer<scalar_t> key_cache_buf(key_cache, key_cache_stride * key_cache_num_tokens);
  sycl::buffer<scalar_t> value_cache_buf(value_cache, key_cache_stride * key_cache_num_tokens);
    ...
    h.parallel_for(sycl::range(num_tokens, num_heads), [=](sycl::item<2> item) {
     ...
     if (slot_idx >= 0) {
       ...
       for (int src_key_idx = 0; src_key_idx < head_size; src_key_idx += x) {
         ...
         for (int i = 0; i < x; ++i) {
           key_cache_acc[target_offset + i + block_elem_num * block_index + head_idx * block_size * head_size] =
       ...
       for (int src_value_idx = 0; src_value_idx < head_size; ++src_value_idx) {
         ...
         value_cache_acc[target_offset + block_elem_num * block_index + head_idx * block_size * head_size] =
  ...
  AT_DISPATCH_FLOATING_TYPES(
      key.scalar_type(), "reshape_and_cache_xpu_impl", [&] {
        reshape_and_cache_xpu_impl<scalar_t>(
            ...
            value_stride, num_heads, head_size, block_size, x, key_cache_stride, key_cache_num_tokens);
      });
}
...
void copy_blocks_xpu(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      key_caches[0].scalar_type(), "copy_blocks_xpu_impl", [&] {
        copy_blocks_xpu_impl<scalar_t>(key_caches, value_caches, mapping_pairs,
