...
typedef void (*fp_gemm_half_q_half_gptq_kernel)(
    ...
    const sycl::half *, sycl::half *, const int,
    const int, const int, const int, const int *,
    const sycl::nd_item<3> &,
    sycl::accessor<sycl::half, 2>);
                                ...
                                int size_n, int size_k, int m_count, int groups)
{
    ...
    */
    ...
    //             kernel(a, b_q_weight, b_gptq_qzeros, b_gptq_scales, c, size_m, size_n,
...
// void gemm_half_q_half_cuda(sycl::queue cublas_handle, const sycl::half *a,
...
//         (const sycl::half*) a.data_ptr(),
//         (const uint32_t*) b_q_weight.data_ptr(),
//         (const uint32_t*)b_gptq_qzeros.data_ptr(),
//         (const sycl::half*) b_gptq_scales.data_ptr(),
//         b_g_idx.device().is_meta() ? NULL : (const int*) b_g_idx.data_ptr(),
//         (sycl::half*) c.data_ptr(),
//         (sycl::half*) temp_dq.data_ptr(),
//         c.size(0),  // m
//         c.size(1),  // n
//         a.size(1),  // k
